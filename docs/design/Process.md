...
exec 以 JSON body 执行一个可执行文件或一段脚本，`format` 区分两种载荷：

| 字段      | 说明                                                                                                             |
| --------- | ---------------------------------------------------------------------------------------------------------------- |
| `format`  | 载荷种类，`exec`（缺省）或 `shell`                                                                               |
| `command` | `format=exec` 必填，可执行 token：bare 名按 PATH（物化目录前置，见 [Binary.md](./Binary.md)）解析，含 `/` 时按路径解析，相对路径相对 `cwd` |
| `args`    | `format=exec` 可选，参数数组，逐项组成 argv                                                                      |
| `script`  | `format=shell` 必填，脚本内容                                                                                    |
| `shell`   | `format=shell` 可选，解释器名或路径，缺省 `sh`；按 PATH 解析                                                     |
