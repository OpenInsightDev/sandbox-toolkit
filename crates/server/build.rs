//! Fetches the prebuilt tool binaries for the target and gzips them into
//! `OUT_DIR` for `tools.rs` to embed. Downloads are cached outside `target/`.
    ...
    match status {
        ...
        Err(error) => panic!(
            "failed to run `curl` while downloading {url}: {error}\n\
             the build needs network access to fetch prebuilt tool binaries"
        ),
    }
