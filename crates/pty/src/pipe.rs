...
/// Spawn a process using regular pipes. The executable path retains its native
/// encoding.
pub async fn spawn_process(
    ...
    spawn_process_with_stdin_mode(program.as_ref(), args, cwd, env, arg0, PipeStdinMode::Piped)
        .await
}

/// Spawn a process using regular pipes and close stdin immediately. The
/// executable path retains its native encoding.
pub async fn spawn_process_no_stdin(
    ...
    spawn_process_with_stdin_mode(program.as_ref(), args, cwd, env, arg0, PipeStdinMode::Null).await
}
